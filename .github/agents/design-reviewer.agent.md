...
- Verify with integrated browser's accessibility features using `read_page` to check semantic structure.

### 3. Responsiveness
Use VS Code's integrated browser to test at each breakpoint, then take a screenshot:
- **Mobile**: 375×812
- **Tablet**: 768×1024
- **Desktop**: 1280×800
...
3. **Navigate to the live page** using VS Code's integrated browser.
4. **Take screenshots** at all three breakpoints by resizing the integrated browser window.
5. **Check accessibility** using `read_page` to analyze semantic structure.
6. **Compare against Penpot** exports if available.
7. **Display in integrated browser** for immediate user review.
