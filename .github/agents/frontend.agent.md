...
5. **Verify with Playwright** — Navigate to the page using `#tool:playwright_browser_navigate`, take screenshots at mobile/tablet/desktop breakpoints, and compare against the design spec.
6. **Open in VS Code browser** — Use `#tool:vscodeRunCommand` with `simpleBrowser.api.open` to show the result to the user.

---

## Verification with Playwright

After implementing changes, always verify visually:

1. **Navigate**: `#tool:playwright_browser_navigate` to the relevant page URL.
2. **Desktop screenshot**: `#tool:playwright_browser_take_screenshot` at default viewport.
3. **Resize to tablet**: `#tool:playwright_browser_resize` to 768×1024, then screenshot.
4. **Resize to mobile**: `#tool:playwright_browser_resize` to 375×812, then screenshot.
5. **Accessibility check**: `#tool:playwright_browser_snapshot` to verify semantic structure.
6. **Interaction test**: Click/hover key interactive elements to verify they work.
